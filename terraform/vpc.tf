...
resource "aws_security_group" "cache_service" {
  name        = "${local.name_prefix}-cache-sg"
  description = "Security group for cache service ECS tasks"
  vpc_id      = aws_vpc.main.id
  ...
  # Raft communication between cache nodes
  ingress {
    description = "Raft inter-node communication"
    from_port   = 11000
    to_port     = 11000
    protocol    = "tcp"
    self        = true
  ...
  # Inter-service HTTP for cluster membership
  ingress {
    description = "HTTP inter-service"
    from_port   = 8080
    to_port     = 8080
    protocol    = "tcp"
    self        = true
...
resource "aws_security_group" "monitoring" {
  ...
  # Prometheus scrape from cache service
  ingress {
